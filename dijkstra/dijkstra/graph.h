...
inline void graph<T>::Dikjstra(graph grafo, string ori)
{
		...
		priority_queue <Point, vector<Point>, myComparator > ColaP; // el primero en salir es el de Distancia 1, luego 2, etc. Se puede  usar mont�culo MIN


		grafo[ori].setDistancia(0);
		ColaP. (G[ori].Dist, ori); // El mont�culo tiene nodos con 2 atributos: Distancia y V�rtice origen

	while (ColaPpq.empty() == false) 
	{

		Point Duo = ColaP.top();	// Duo tiene una Distancia y el v�rtice
		string Vi = Duo.getY();  // El V�rtice es el segundo elemento
